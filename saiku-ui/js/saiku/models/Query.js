...
var Query = Backbone.Model.extend({
  ...
  initialize: function(args, options) {
    ...
    this.uuid = 'xxxxxxxx-xxxx-xxxx-yxxx-xxxxxxxxxxxx'
    ...
    if (this.updatedSelectionFromModal) {
      ...
      for (var mKey in mParameters) {
        ...
        if (mVal !== null && mVal !== undefined) {
          this.helper.setSelectionsForParameter(
            mKey,
            _.filter(selections, function(sel) {
  ...
  run: function(force, mdx) {
    ...
    Saiku.ui.unblock();
    if (
      typeof this.model.properties !== 'undefined' &&
			this.model.properties['saiku.olap.query.automatic_execution'] === false &&
    ...
    if (exModel.queryType == 'OLAP') {
      if (exModel.type == 'QUERYMODEL') {
        ...
        if (!columnsOk && !detailsOk) {
          errorMessage +=
						'<span class="i18n">You need to include at least one measure or a level on columns for a valid query.</span>';
        }
        if (!rowsOk) {
    ...
    $(this.workspace.processing)
      .html(
        '<span class="processing_image">&nbsp;&nbsp;</span> <span class="i18n">Running query...</span> [&nbsp;<a class="cancel i18n" href="#cancel">Cancel</a>&nbsp;]'
      )
      .show();
    this.workspace.adjust();
...
*/
    ...
    this.result.save(
      {},
      {
        ...
        data: JSON.stringify(exModel),
        error: function() {
          Saiku.ui.unblock();
          var errorMessage =
						'<span class="i18n">Error executing query. Please check the server logs or contact your administrator!</span>';
