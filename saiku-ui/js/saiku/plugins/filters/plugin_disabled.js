...
var Filters = Backbone.View.extend({
  ...
  add_button: function() {
    var $stats_button = $('<a href="#filters" class="filters button disabled_toolbar i18n" title="Filters"></a>').css({
      'background-image': "url('js/saiku/plugins/filters/tag_red.png')",
...
*/
  ...
  deactivate_add_filter: function(other) {
    ...
    $(self.el)
      .find('a.add_filter, a.edit_filter')
      .removeClass('on');
    $(self.el)
      .find('.new_filter_input, .edit_filter_input, .new_filter')
      .parent()
      .remove();
    return false;
  },
  ...
  add_filter: function(event) {
    ...
    this.workspace.query.action.gett('/filter', {
      success: function(model, response) {
        if (response != null && response.dimension != null && response.members.length > 0) {
          ...
          alert(
            'Filter cannot be created. Dimension: ' +
							FilterConfig.dimension +
							' not part of the query or no matching members found!'
          );
          self.deactivate_add_filter();
        }
      },
    ...
    if (queryFilter != null) {
      ...
      if (filterAction == 'add') {
        $(
          "<li><input id='new_filter' type='text' class='new_filter_input'/> &nbsp; (" +
						queryFilter.members.length +
						') &nbsp; </li>' +
						"<li><a href='#save_filter' class='i18n save sprite button new_filter' title='Save Filter'></a></li>"
        ).insertAfter(
          $(self.el)
            .find('.filter_items .add_filter')
            .parent()
        );
      }
      else {
        ...
        _.each(self.filters, function(filter) {
          edit +=
						"<option value='" + safe_tags_replace(filter.name) + "'>" + safe_tags_replace(filter.name) + '</option>';
        });
