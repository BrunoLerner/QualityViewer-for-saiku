...
var Buckets = Backbone.View.extend({
  ...
  add_button: function() {
    var $stats_button = $('<a href="#buckets" class="buckets button disabled_toolbar i18n" title="Tags"></a>').css({
  ...
  save_bucket: function() {
    ...
    var saveBucket = function(model, response) {
      ...
      self.deactivate_add_bucket();
      $tag = $(self.tag_template(model)).appendTo($(self.el).find('.bucket_items ul'));
    };
