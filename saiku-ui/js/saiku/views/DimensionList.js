...
var DimensionList = Backbone.View.extend({
  ...
  render_attributes: function() {
    ...
    $(this.el)
      .find('.addMeasure, .calculated_measures')
      .show();
    $(this.el)
      .find('.measure')
      .parent('li')
    ...
    $(this.el)
      ...
      .draggable({
        ...
        connectToSortable: $(this.workspace.el).find(
          '.fields_list_body.columns > ul.connectable, .fields_list_body.rows > ul.connectable, .fields_list_body.filter > ul.connectable'
        ),
        containment: $(self.workspace.el),
        // helper: "clone",
        helper: function(event, ui) {
          ...
          h.find('li a[hierarchy="' + hierarchy + '"]')
            .parent()
            .hide();
  ...
  select_dimension: function(event, ui) {
    ...
    var hierarchyCaption = $(event.target)
      .parent()
      .parent()
