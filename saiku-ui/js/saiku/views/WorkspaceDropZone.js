...
var WorkspaceDropZone = Backbone.View.extend({
  ...
  set_measures: function(event, ui) {
    ...
    $(this.el)
      .find('.fields_list_body.details ul.connectable li.d_measure')
      .each(function(index, element) {
        ...
        Saiku.events.trigger('workspaceDropZone:select_measure', this, { measure: measure });
      });
  ...
  remove_dimension: function(event, ui) {
    if ($(ui.helper).hasClass('d_measure')) {
      ...
      var hierarchy = $(ui.helper)
        .find('a')
        .attr('hierarchy');
      var fromAxis = $(this.el)
        .find('ul.hierarchy[hierarchy="' + hierarchy + '"]')
        .parents('.fields_list')
        .attr('title');
      var level = $(ui.helper)
        .find('a')
  ...
  synchronize_query: function() {
    ...
    if (model.hasOwnProperty('queryModel') && model.queryModel.hasOwnProperty('axes')) {
      ...
      for (var axis in axes) {
        ...
        _.each(axes[axis].hierarchies, function(hierarchy) {
          if (self.workspace.dimension_list != null) {
            var h = $(self.workspace.dimension_list.el)
              .find('ul.d_hierarchy[hierarchy="' + hierarchy.name + '"]')
              .clone()
              .removeClass('d_hierarchy')
              .addClass('hierarchy');

            h.find('li.d_level').hide();
            for (var level in hierarchy.levels) {
              h.find('li a[level="' + level + '"]')
                .parent()
                .show();

              // sync attribute list
              $(self.workspace.dimension_list.el)
                .find('ul.d_hierarchy[hierarchy="' + hierarchy.name + '"] li a[level="' + level + '"]')
                .parent()
                .draggable('disable')
      ...
      _.each(measures, function(measure) {
        if (self.workspace.dimension_list != null) {
          var m = $(self.workspace.dimension_list.el)
            .find('.measure_tree a.measure[measure="' + measure.name + '"]')
            .parent();
          var m2 = m.clone().show();
  ...
  clear_axis: function(event) {
    ...
    var axisName = $(event.target)
      .siblings('.fields_list_body')
      .parent()
      .attr('title');
      ...
      for (var i = 0; i < len; i++) {
        for (var lName in axisData.hierarchies[i].levels) {
          if (axisData.hierarchies[i].levels.hasOwnProperty(lName)) {
            if (
              axisData.hierarchies[i].levels[lName].selection &&
  ...
  selections: function(event, ui) {
    ...
    // Determine dimension
    var $target = $(event.target).hasClass('d_level') ? $(event.target).find('.level') : $(event.target);
    ...
    if (
      objData.level &&
			objData.level.annotations !== undefined &&
			...
			((_.has(memberLevel, 'selection') && memberLevel.selection.members.length === 0) ||
				((_.size(memberLevel) === 1 && _.has(memberLevel, 'name')) ||
					(_.has(memberLevel, 'mdx') && memberLevel.mdx) ||
  ...
  measure_action: function(event) {
    ...
    if (
      typeof this.workspace.query === 'undefined' ||
    ...
    $.contextMenu({
      ...
      build: function($trigger, e) {
        ...
        var addFun = function(items, fun) {
          ...
          for (var key in items) {
            ...
            if (fun == func && sortliteral == key && items[key].payload.n == n) {
              topHl = fun + 'Quick';
              ret[fun + '###SEPARATOR###' + key].name = '<b>' + items[key].name + '</b>';
            }
        ...
        _.each(selectedMeasures, function(measure) {
          ...
          fold.items['show_totals_nil_' + measure.name] = {
            name: self.formatAggregatorName('None', 'nil', a, measure),
            i18n: true
          };
          fold.items['show_totals_sum_' + measure.name] = {
            name: self.formatAggregatorName('Sum', 'sum', a, measure),
            i18n: true
          };
          fold.items['show_totals_min_' + measure.name] = {
            name: self.formatAggregatorName('Min', 'min', a, measure),
            i18n: true
          };
          fold.items['show_totals_max_' + measure.name] = {
            name: self.formatAggregatorName('Max', 'max', a, measure),
            i18n: true
          };
          fold.items['show_totals_avg_' + measure.name] = {
            name: self.formatAggregatorName('Avg', 'avg', a, measure),
            i18n: true
          };
        ...
        return {
          callback: function(key, options) {
            var save_custom, save_customsort;
