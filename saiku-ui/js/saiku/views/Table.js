...
var Table = Backbone.View.extend({
      ...
      if ($rowIterator.length > 0) {
        ...
        if (isLast.length > 0 && $(isLast[col]).hasClass('row_total_corner')) {
          ...
          for (var i = 0; i <= col; i++) {
            ...
            if (
              row < self.workspace.query.result.lastresult().cellset.length &&
							self.workspace.query.result.lastresult().cellset[row][i].value !=
								self.workspace.query.result.lastresult().cellset[row - 1][i].value &&
							!thisIsTotal
            ) {
  ...
  expand: function(event, $targetSpan, row, col) {
    ...
    while (
      $rowIterator.length > 0 &&
			($($rowIterator.find('th,td')[col]).hasClass('row_null') ||
				$($rowIterator.find('th,td')[col]).hasClass('row_total_corner'))
    ) {
      $rowIterator
        .find('span.collapsed')
        .removeClass('collapsed')
  ...
  clicked_cell: function(event) {
    ...
    var $target =
			$(event.target).hasClass('row') || $(event.target).hasClass('col') ?
    ...
    $.contextMenu({
      ...
      build: function($trigger, e) {
        ...
        if (v1) {
          v2 = _.each(v1.levels, function(level) {
