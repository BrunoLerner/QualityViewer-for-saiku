...
var WorkspaceToolbar = Backbone.View.extend({
  ...
  initialize: function(args) {
    ...
    _.bindAll(
      ...
      'display_drillthrough',
      'clicked_cell_drillthrough_export',
      'clicked_cell_drillacross',
      'clicked_cell_drillthrough',
      'activate_buttons',
      'switch_to_mdx',
      'post_mdx_transform',
      'toggle_fields_action',
      'group_parents'
    );
  ...
  activate_buttons: function(args) {
    if (args !== null && args.data && args.data.cellset && args.data.cellset.length > 0) {
      ...
      $(args.workspace.toolbar.el)
        ...
        .removeClass('on');
      $(args.workspace.el)
        .find('.fields_list .disabled_toolbar')
        .removeClass('disabled_toolbar');
      $(args.workspace.toolbar.el)
        .find(
          '.about, .new, .open, .save, .edit, .run,.auto,.non_empty,.toggle_fields,.toggle_sidebar,.switch_to_mdx, .mdx'
        )
        .removeClass('disabled_toolbar');
    }
  ...
  reflect_properties: function() {
    var properties = this.workspace.query.model.properties ?
      this.workspace.query.model.properties :
    ...
    if (properties['saiku.olap.query.drillthrough'] !== true) {
      $(this.el)
        .find('.drillthrough, .drillthrough_export')
        .addClass('disabled_toolbar');
    }
    ...
    if (properties['saiku.olap.query.limit'] == 'true' || properties['saiku.olap.query.filter'] === true) {
      $(this.workspace.el)
        .find('.fields_list_header')
        .addClass('limit');
    }

    if (
      this.workspace.query.getProperty('saiku.olap.result.formatter') !== 'undefined' &&
			this.workspace.query.getProperty('saiku.olap.result.formatter') == 'flattened'
    ) {
      if (
        !$(this.el)
          .find('.group_parents')
    ...
    if (!$(source).hasClass('on')) {
      ...
      if ($(source).hasClass('drillthrough_export')) {
        $(this.workspace.el)
          ...
          .unbind('click')
          .click(this.clicked_cell_drillthrough_export);
        $(this.workspace.el)
          .find('.query_scenario, .drillthrough, .zoom_mode, .drillacross')
          .removeClass('on');
      }
        ...
        $(this.workspace.el)
          ...
          .addClass('cellhighlight')
          .unbind('click')
          .click(this.clicked_cell_drillthrough);
        $(this.workspace.el)
          .find('.query_scenario, .drillthrough_export, .zoom_mode, .drillacross')
          .removeClass('on');
      }
      else if ($(source).hasClass('query_scenario')) {
        this.workspace.query.scenario.activate();
        $(this.workspace.el)
          .find('.drillthrough, .drillthrough_export, .zoom_mode, .drillacross')
          .removeClass('on');
      }
        ...
        $(this.workspace.el)
          ...
          .addClass('cellhighlight')
          .unbind('click')
          .click(this.clicked_cell_drillacross);
        $(this.workspace.el)
          .find('.query_scenario, .drillthrough, .drillthrough_export, .zoom_mode')
          .removeClass('on');
      }
          ...
          .selectable({
            ...
            stop: function(event, ui) {
              ...
              if (positions.length > 0) {
                self.workspace.query.action.put('/zoomin', {
        ...
        $(this.workspace.el)
          .find('.drillthrough, .drillthrough_export, .query_scenario, .drillacross, .about')
          .removeClass('on');
      }
    }
  },
  ...
  clicked_cell_drillacross: function(event) {
    $target = $(event.target).hasClass('data') ? $(event.target).find('div') : $(event.target);
  ...
  switch_to_mdx: function(event) {
    ...
    $(this.workspace.el)
      ...
      .addClass('hide');
    $(this.el)
      .find('.auto, .query_scenario, .buckets, .non_empty, .swap_axis, .mdx, .switch_to_mdx, .zoom_mode, .drillacross')
      .parent()
      .hide();
    ...
    if (Settings.MODE != 'view' && Settings.MODE != 'table' && !this.workspace.isReadOnly) {
      ...
      $(this.workspace.el)
        .find('.workspace_editor .mdx_input, .workspace_editor .editor_info, .workspace_editor')
        .removeClass('hide')
        .show();
      ...
      var heightUpdateFunction = function() {
        ...
        var screen_length =
					self.editor.getSession().getScreenLength() > height ? height : self.editor.getSession().getScreenLength();
        var newHeight =
					(screen_length + 1) * self.editor.renderer.lineHeight + self.editor.renderer.scrollBar.getWidth();
    ...
    if (this.workspace.dimension_list) {
      $(this.workspace.el)
        .find('.sidebar_inner ul li a')
        .css({ fontWeight: 'normal' })
        .parent('li')
        .removeClass('ui-draggable ui-draggable-disabled ui-state-disabled');
    }
  ...
  post_mdx_transform: function() {
    ...
    if (Settings.ALLOW_PARAMETERS) {
      ...
      var lazyDetector = function() {
        _.delay(parameterDetector, 1000);
      };
  ...
  explain_query: function(event) {
    ...
    var explained = function(model, args) {
      var explainPlan =
				"<textarea style='width: " + ($('body').width() - 165) + 'px;height:' + ($('body').height() - 175) + "px;'>";
      ...
      var html =
				'<div id="fancy_results" class="workspace_results" style="overflow:visible"><table>' +
