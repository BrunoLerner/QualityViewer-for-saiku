...
var SelectionsModal = Modal.extend({
  ...
  initialize: function(args) {
    ...
    if (
      (this.source === 'DateFilterModal' && (_.has(level, 'selection') && level.selection.members.length === 0)) ||
    ...
    $(this.el)
      .find('#use_result')
      .attr('checked', this.use_result_option);
    $(this.el)
      .find('.search_limit')
      .text(this.members_search_limit);
    $(this.el)
      .find('.members_limit')
      .text(this.members_limit);
  ...
  init_totals: function(hierarchy, level) {
    ...
    for (var j = 0; j < measuresArray.length; j++) {
      $(this.el)
        .find('#div-totals-container')
        .append(_.template($('#template-selections-totals').html())({ measure: measuresArray[j] }));
    }

    if (
      _.size(hierarchy.levels) > 1 &&
			level &&
			level.hasOwnProperty('aggregators') &&
			level.aggregators &&
			level.aggregators.length > 0
    ) {
    ...
    if (this.show_totals_option.length > 0) {
      if (this.per_metrics_totals) {
        ...
        $(this.el)
          ...
          .val(self.show_totals_option[0]);
      }
    }
    $(this.el)
      .find('#per_metrics_totals_checkbox')
      .attr('checked', this.per_metrics_totals);
  },
  ...
  per_metrics_totals_action: function(event) {
    ...
    if (this.per_metrics_totals === true) {
      $(this.el)
        .find('.per_metrics_container')
        .show();
      $(this.el)
        .find('.all_metrics_container')
        .hide();
    }
    else {
      $(this.el)
  ...
  get_members: function() {
    var self = this;
    var path =
			'/result/metadata/hierarchies/' +
			encodeURIComponent(this.member.hierarchy) +
			'/levels/' +
			encodeURIComponent(this.member.level);
  ...
  search_members: function() {
    ...
    var message =
			'<span class="processing_image">&nbsp;&nbsp;</span> <span class="i18n">Searching for members matching:</span> ' +
			search_term;
  ...
  populate: function(model, response) {
    ...
    self.workspace.unblock();
    this.members_search_server =
			this.available_members.length >= this.members_limit || this.available_members.length == 0;

    self.show_unique_option = false;
    $(this.el)
      .find('.options #show_unique')
      .attr('checked', false);
    ...
    $(this.el)
      .find('.items_size')
      .text(this.available_members.length);

    if (this.members_search_server) {
      $(this.el)
        .find('.warning')
        .text('More items available than listed. Pre-Filter on server.');
    }
    else {
      $(this.el)
        .find('.warning')
    ...
    $(self.el)
      ...
      .selectable({
        ...
        stop: function(event, ui) {
          $(self.el)
            .find('.selection_options li.ui-selected input')
            .each(function(index, element) {
              ...
              $(element)
                .parents('.selection_options')
                .find('li.all_options input')
                .prop('checked', true);
            });
          $(self.el)
            .find('.selection_options li.ui-selected')
    ...
    $(this.el)
      ...
      .autocomplete({
        ...
        select: function(event, ui) {
          ...
          $(self.el)
            .find('.available_selections .selection_options input[value="' + encodeURIComponent(searchVal) + '"]')
            .parent()
            .remove();
          $(self.el)
            .find('.used_selections .selection_options input[value="' + encodeURIComponent(searchVal) + '"]')
            .parent()
            .remove();

          var option =
						'<li class="option_value"><input type="checkbox" class="check_option" value="' +
  ...
  move_selection: function(event) {
    ...
    var $from =
			action.indexOf('add') !== -1 ?
			  ...
			  $(this.el).find('.used_selections .selection_options ul');
    var $els =
			action.indexOf('all') !== -1 ?
			  $from.find('li.option_value input').parent() :
			  $from.find('li.option_value input:checked').parent();

    $els.detach().appendTo($to);
    $(this.el)
      .find('.selection_options ul li.option_value input:checked')
      .prop('checked', false);
    $(this.el)
      .find('.selection_options li.all_options input')
      .prop('checked', false);
  },
  ...
  click_move_selection: function(event, ui) {
    ...
    var to = $(event.target)
      ...
      .parent()
      .parent()
      .hasClass('used_selections') ?
      '.available_selections' :
      '.used_selections';

    $(event.target)
      .parent()
      .appendTo($(this.el).find(to + ' .selection_options ul'));
  },
  ...
  show_unique_action: function() {
    ...
    if (this.show_unique_option === true) {
      $(this.el)
        .find('.available_selections, .used_selections')
        .addClass('unique');
      $(this.el)
        .find('.available_selections, .used_selections')
        .removeClass('caption');
  ...
  save: function() {
    ...
    if ($(this.el).find('.used_selections input').length === 0) {
      ...
      $(this.el)
        .find('.used_selections .option_value input')
    ...
    if (hierarchy && hierarchy.levels.hasOwnProperty(lName)) {
      ...
      var selectionType = $(self.el)
        .find('input.selection_type:checked')
        .val();
